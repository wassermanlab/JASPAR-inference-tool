...
def get_profiles(devel=False, output_dir="./"):
    ...
    for taxon in taxons:
        ...
        if not os.path.exists(taxon_dir):
            ...
            # Get JASPAR profiles
            url = "http://jaspar.genereg.net/download/CORE/"
            file_name = "JASPAR2018_CORE_%s_redundant_pfms_meme.zip" % taxon
            if devel:
                url = "http://hfaistos.uio.no:8002/download/CORE/"
                file_name = "JASPAR2020_CORE_%s_redundant_pfms_meme.zip" % taxon
