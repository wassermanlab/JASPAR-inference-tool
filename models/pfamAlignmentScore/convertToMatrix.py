...
def getDiffChar(string1,string2): # return the positions where the string differs
    ...
    if (len(string1) == len(string2)):
        ...
        for i in range(len(string1)):
            if string1[i] == string2[i] and string1[i] != '-':
                continue # if the case match and are not blank lines
            else:
                diff.append(i) # NOTE: diff gives index starting from 0
    else:
...
def scoringID(testseq1,testseq2): # called for each comparison to compare the string
    ...
    positions = getDiffChar(testseq1,testseq2)
    
    # The default score needs to be changed 
    positionScore = np.ones(len(testseq1)) # default score
    positionScore[positions] = 0 # where they are different, ID score = 0
    print(positionScore)
    return(positionScore.tolist()) # returns an list of 1 or 0 (on the length)
...
for key,item in alignD.items(): # key does not have version .xx number
    ...
    #exit(0)
# storing the matrix...
with open(file_header+"/identityScoreMatrix.json",'w+') as wf:
